...
class Agent(object):
	...
	def load(self, av_checkpoint_path):
		"""
        Load the policy network model weights from a checkpoint file.

        Parameters:
        av_checkpoint_path (str): Path to the checkpoint file to load the weights.
        """
		try:
			# Load the model weights into policy_net
			policy_net_weights = torch.load(av_checkpoint_path, map_location=device)
			self.policy_net.load_state_dict(policy_net_weights)
			print(f"Policy network successfully loaded from {av_checkpoint_path}")
		except Exception as e:
			print(f"Error loading checkpoint from {av_checkpoint_path}: {e}")
