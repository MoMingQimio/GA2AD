...
class Agent(object):
	...
	def __init__(self, arg, hp=""):
		...
		self.writter = SummaryWriter(log_dir=log_dir)  # 修改，指定日志文件夹
	...
	def train_RL(self, env):
		...
		# 在一次强化学习任务后初始化网络参数以确保每次迭代开始时都使用相同的初始权重
		self.weights_init(self.policy_net)  # 每次迭代重新初始化网络参数
		self.target_net.load_state_dict(self.policy_net.state_dict())  # 每次迭代开始时，同步target网络参数
		ep = 10
		for e in range(ep):
			isGui = False
			if (e + 1) % 100 == 0:  # 这两行注释掉每一回合都会出现sumo界面
				isGui = True
			...
			for t in count():  # 创建一个从start开始每次的步长是step的无穷序列 #count()|	[start[, step]]|	start, start+step,
				# start+2*step, … |	count(10) --> 10 11 12 13
				...
				if done:
					...
					recent_rewards.append(r_r)
					if len(recent_rewards) > 100:
						recent_rewards.pop(0)
					self.mean_reward = sum(recent_rewards) / len(recent_rewards)
			...
			self.writter.add_scalar("Mean Reward (Last 100)/Train", self.mean_reward, (e + 1))
			self.writter.flush()
